...
def visualize_confidence_level(prediction_proba):
    ...
    grad_percentage = pd.DataFrame(data = data,columns = ['Percentage'],index = ['Bajo','Medio','Alto'])
    ...
    ax.set_xlabel(" Porcentaje(%) Nivel de confianza", labelpad=2, weight='bold', size=12)
...
Esta aplicación predice la **calidad del vino** mediante la entrada de **características** del vino
""")
...
st.sidebar.header('Parámetros de entrada') #user input parameter collection with streamlit side bar
...
def get_user_input():
    ...
    fixed_acidity = st.sidebar.slider('Acidez fija', 3.8, 15.9, 7.0)
    volatile_acidity = st.sidebar.slider('Acidez volátil', 0.08, 1.58, 0.4)
    citric_acid  = st.sidebar.slider('Ácido cítrico', 0.0, 1.66, 0.3)
    residual_sugar  = st.sidebar.slider('Azúcar residual', 0.6, 65.8, 10.4)
    chlorides  = st.sidebar.slider('Cloruros', 0.009, 0.611, 0.211)
    free_sulfur_dioxide = st.sidebar.slider('Dióxido de azufre libre', 1, 289, 200)
    total_sulfur_dioxide = st.sidebar.slider('Dióxido de azufre total', 6, 440, 150)
    density = st.sidebar.slider('Densidad', 0.98, 1.03, 1.0)
    pH = st.sidebar.slider('pH', 2.72, 4.01, 3.0)
    sulphates = st.sidebar.slider('Sulfatos', 0.22, 2.0, 1.0)
